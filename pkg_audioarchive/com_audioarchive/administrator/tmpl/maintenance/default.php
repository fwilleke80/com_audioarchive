...
$checkUrl = static fn(string $check): string => Route::_('index.php?option=com_audioarchive&view=maintenance&check=' . $check);
?>
<div class="com-audioarchive-maintenance">
		...
		<div class="card-body">
			<?php foreach ([
				[
					...
					'regenerate' => 'COM_AUDIOARCHIVE_WAVEFORM_REGENERATE_ALL',
					'regenerate_confirm' => 'COM_AUDIOARCHIVE_WAVEFORM_REGENERATE_ALL_CONFIRM',
					'delete_task' => 'maintenance.deleteWaveforms',
					'delete' => 'COM_AUDIOARCHIVE_WAVEFORM_DELETE_ALL',
					'delete_confirm' => 'COM_AUDIOARCHIVE_WAVEFORM_DELETE_ALL_CONFIRM',
				...
				[
					...
					'regenerate' => 'COM_AUDIOARCHIVE_SPECTROGRAM_REGENERATE_ALL',
					'regenerate_confirm' => 'COM_AUDIOARCHIVE_SPECTROGRAM_REGENERATE_ALL_CONFIRM',
					'delete_task' => 'maintenance.deleteSpectrograms',
					'delete' => 'COM_AUDIOARCHIVE_SPECTROGRAM_DELETE_ALL',
				...
				<section class="<?php echo $analysisSection['type'] === 'spectrogram' ? 'border-top pt-4 mt-4' : ''; ?>">
					...
					<div class="row g-3 mb-3">
						<?php foreach (['available', 'missing', 'pending', 'failed', 'stale', 'queued'] as $key) : ?>
							<div class="col-6 col-md-4 col-xl">
								<div class="border rounded p-3 h-100">
									<div class="h3 mb-1"><?php echo (int) ($analysisSummary[$key] ?? 0); ?></div>
						...
						<div class="col-6 col-md-4 col-xl">
							<div class="border rounded p-3 h-100">
								<div class="h3 mb-1"><?php echo htmlspecialchars($formatBytes((int) ($analysisSummary['disk_usage'] ?? 0)), ENT_QUOTES, 'UTF-8'); ?></div>
								<div class="small text-body-secondary"><?php echo Text::_('COM_AUDIOARCHIVE_ANALYSIS_DISK_USAGE'); ?></div>
							</div>
						</div>
					</div>
					...
					<div class="d-flex flex-wrap gap-2 align-items-start">
						<form action="<?php echo Route::_('index.php?option=com_audioarchive'); ?>" method="post" class="d-flex flex-wrap gap-2">
							...
							<button type="submit" name="<?php echo htmlspecialchars($analysisSection['mode_name'], ENT_QUOTES, 'UTF-8'); ?>" value="missing" class="btn btn-outline-primary" <?php echo (int) ($analysisSummary['missing'] ?? 0) <= 0 ? 'disabled' : ''; ?>>
								<?php echo Text::_($analysisSection['queue_missing']); ?>
							</button>
							<button type="submit" name="<?php echo htmlspecialchars($analysisSection['mode_name'], ENT_QUOTES, 'UTF-8'); ?>" value="stale" class="btn btn-outline-primary" <?php echo (int) ($analysisSummary['stale'] ?? 0) <= 0 ? 'disabled' : ''; ?>>
								<?php echo Text::_($analysisSection['queue_stale']); ?>
							</button>
							<button type="submit" name="<?php echo htmlspecialchars($analysisSection['mode_name'], ENT_QUOTES, 'UTF-8'); ?>" value="failed" class="btn btn-outline-primary" <?php echo (int) ($analysisSummary['failed'] ?? 0) <= 0 ? 'disabled' : ''; ?>>
								<?php echo Text::_($analysisSection['retry_failed']); ?>
							</button>
							<button
								type="submit"
								name="<?php echo htmlspecialchars($analysisSection['mode_name'], ENT_QUOTES, 'UTF-8'); ?>"
								value="all"
								class="btn btn-warning"
								onclick="return confirm(<?php echo htmlspecialchars(json_encode(Text::_($analysisSection['regenerate_confirm'])), ENT_QUOTES, 'UTF-8'); ?>);"
							>
								<span class="icon-refresh" aria-hidden="true"></span>
								<?php echo Text::_($analysisSection['regenerate']); ?>
							</button>
							<?php echo HTMLHelper::_('form.token'); ?>
						</form>
						<?php if ($canManageFiles) : ?>
							<form action="<?php echo Route::_('index.php?option=com_audioarchive'); ?>" method="post">
								<input type="hidden" name="task" value="<?php echo htmlspecialchars($analysisSection['delete_task'], ENT_QUOTES, 'UTF-8'); ?>">
								<button
									type="submit"
									class="btn btn-danger"
									<?php echo $recordCount <= 0 && (int) ($analysisSummary['disk_usage'] ?? 0) <= 0 ? 'disabled' : ''; ?>
									onclick="return confirm(<?php echo htmlspecialchars(json_encode(Text::_($analysisSection['delete_confirm'])), ENT_QUOTES, 'UTF-8'); ?>);"
								>
									<span class="icon-trash" aria-hidden="true"></span>
									<?php echo Text::_($analysisSection['delete']); ?>
								</button>
								<?php echo HTMLHelper::_('form.token'); ?>
							</form>
						<?php endif; ?>
	...
	<div class="row g-3 mb-4">
		<?php foreach ([
			['id' => 'integrity', 'title' => 'COM_AUDIOARCHIVE_MAINTENANCE_CHECK_INTEGRITY_TITLE', 'text' => 'COM_AUDIOARCHIVE_MAINTENANCE_CHECK_INTEGRITY_TEXT'],
			['id' => 'codecs', 'title' => 'COM_AUDIOARCHIVE_MAINTENANCE_CHECK_CODECS_TITLE', 'text' => 'COM_AUDIOARCHIVE_MAINTENANCE_CHECK_CODECS_TEXT'],
			['id' => 'stale', 'title' => 'COM_AUDIOARCHIVE_MAINTENANCE_CHECK_STALE_TITLE', 'text' => 'COM_AUDIOARCHIVE_MAINTENANCE_CHECK_STALE_TEXT'],
		] as $check) : ?>
			<div class="col-12 col-lg-4">
				<div class="card h-100<?php echo $checkedSection === $check['id'] ? ' border-primary' : ''; ?>">
					<div class="card-body d-flex flex-column">
						<h2 class="h5"><?php echo Text::_($check['title']); ?></h2>
						<p class="text-body-secondary flex-grow-1"><?php echo Text::_($check['text']); ?></p>
						<a class="btn <?php echo $checkedSection === $check['id'] ? 'btn-primary' : 'btn-outline-primary'; ?> align-self-start" href="<?php echo $checkUrl($check['id']); ?>">
							<?php echo Text::_($checkedSection === $check['id'] ? 'COM_AUDIOARCHIVE_MAINTENANCE_CHECK_AGAIN' : 'COM_AUDIOARCHIVE_MAINTENANCE_CHECK_NOW'); ?>
						</a>
					</div>
				</div>
			</div>
					...
					<table class="table table-striped align-middle mb-0">
						...
						<tbody>
							<?php foreach ($codecInventory as $row) : ?>
								...
								<tr>
									<th scope="row">
										<a href="<?php echo Route::_('index.php?option=com_audioarchive&view=maintenance&check=codecs&codec=' . rawurlencode($filterValue)); ?>">
											<?php echo (string) $row['codec'] !== '' ? htmlspecialchars((string) $row['codec'], ENT_QUOTES, 'UTF-8') : Text::_('COM_AUDIOARCHIVE_MAINTENANCE_UNKNOWN_CODEC'); ?>
										</a>
									</th>
				...
				<div class="card-header border-top d-flex flex-wrap justify-content-between align-items-center gap-2">
					<h3 class="h5 mb-0">
						<?php echo Text::sprintf(
							...
							$codecFilter === '__unknown__' ? Text::_('COM_AUDIOARCHIVE_MAINTENANCE_UNKNOWN_CODEC') : htmlspecialchars($codecFilter, ENT_QUOTES, 'UTF-8')
						); ?>
					</h3>
					<a class="btn btn-sm btn-outline-secondary" href="<?php echo $checkUrl('codecs'); ?>"><?php echo Text::_('JCLEAR'); ?></a>
				</div>
				<?php if ($codecClips === []) : ?>
						...
						<table class="table align-middle mb-0">
							...
							<tbody>
								<?php foreach ($codecClips as $clip) : ?>
									...
									?>
									<tr>
										<th scope="row"><a href="<?php echo Route::_('index.php?option=com_audioarchive&task=clip.edit&id=' . (int) $clip->id); ?>"><?php echo htmlspecialchars((string) $clip->title, ENT_QUOTES, 'UTF-8'); ?></a><div class="small text-body-secondary">#<?php echo (int) $clip->id; ?></div></th>
										<td><?php echo htmlspecialchars((string) $clip->original_filename, ENT_QUOTES, 'UTF-8'); ?></td>
										<td><?php echo htmlspecialchars((string) ($clip->container_format ?: '—'), ENT_QUOTES, 'UTF-8'); ?> <code><?php echo htmlspecialchars((string) ($clip->file_extension ?: ''), ENT_QUOTES, 'UTF-8'); ?></code></td>
		...
		<form action="<?php echo Route::_('index.php?option=com_audioarchive'); ?>" method="post" id="adminForm" name="adminForm">
			<div class="card mb-4" id="audioarchive-stale-files">
				<div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
					...
					<?php if ($staleItems !== [] && $canManageFiles) : ?>
						<button type="button" id="audioarchive-delete-stale" class="btn btn-danger"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_STALE_DELETE_SELECTED'); ?></button>
					<?php endif; ?>
				</div>
				<div id="audioarchive-stale-progress" class="alert alert-info m-3 d-none" role="status" aria-live="polite"></div>
				<?php if ($staleItems === []) : ?>
					<div class="card-body"><div class="alert alert-success mb-0"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_STALE_EMPTY'); ?></div></div>
				<?php else : ?>
					<div class="table-responsive">
						<table class="table align-middle mb-0">
							<thead><tr>
								<th scope="col" class="w-1 text-center"><input type="checkbox" class="form-check-input" aria-label="<?php echo Text::_('JGLOBAL_CHECK_ALL'); ?>" onclick="document.querySelectorAll('input[name=&quot;stale[]&quot;]').forEach((item) => item.checked = this.checked);"></th>
							...
							<tbody>
								<?php foreach ($staleItems as $item) : ?>
									<tr>
										<td class="text-center"><input type="checkbox" class="form-check-input" name="stale[]" value="<?php echo htmlspecialchars((string) $item['token'], ENT_QUOTES, 'UTF-8'); ?>" aria-label="<?php echo htmlspecialchars((string) $item['storage_key'], ENT_QUOTES, 'UTF-8'); ?>"></td>
										...
										<td>
											<?php if ((int) $item['clip_id'] > 0) : ?>
												<a href="<?php echo Route::_('index.php?option=com_audioarchive&task=clip.edit&id=' . (int) $item['clip_id']); ?>"><?php echo htmlspecialchars((string) ($item['clip_title'] ?: '#' . $item['clip_id']), ENT_QUOTES, 'UTF-8'); ?></a>
												<?php if ((string) $item['original_filename'] !== '') : ?><div class="small text-body-secondary"><?php echo htmlspecialchars((string) $item['original_filename'], ENT_QUOTES, 'UTF-8'); ?></div><?php endif; ?>
											<?php else : ?><span class="text-body-secondary"><?php echo Text::_('JNONE'); ?></span><?php endif; ?>
										</td>
	...
	<?php if ($checkedSection === 'integrity') : ?>
		<div class="row g-3 mb-4">
			<?php foreach ([
				['clips', 'COM_AUDIOARCHIVE_MAINTENANCE_SUMMARY_CLIPS'],
				['original_records', 'COM_AUDIOARCHIVE_MAINTENANCE_SUMMARY_ORIGINAL_RECORDS'],
				['managed_original_files', 'COM_AUDIOARCHIVE_MAINTENANCE_SUMMARY_MANAGED_FILES'],
				['errors', 'COM_AUDIOARCHIVE_MAINTENANCE_SUMMARY_ERRORS'],
				['warnings', 'COM_AUDIOARCHIVE_MAINTENANCE_SUMMARY_WARNINGS'],
				['issues', 'COM_AUDIOARCHIVE_MAINTENANCE_SUMMARY_ISSUES'],
			] as [$key, $label]) : ?>
				<div class="col-6 col-lg-2"><div class="card h-100"><div class="card-body"><div class="display-6"><?php echo (int) ($summary[$key] ?? 0); ?></div><div><?php echo Text::_($label); ?></div></div></div></div>
			<?php endforeach; ?>
		</div>
		...
		<form action="<?php echo Route::_('index.php?option=com_audioarchive'); ?>" method="post" id="adminForm" name="adminForm">
			<?php if ($actionableClips !== []) : ?>
				<div class="card mb-4">
					<div class="card-header"><h2 class="h4 mb-1"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_ACTIONS_TITLE'); ?></h2><p class="mb-0 text-body-secondary"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_ACTIONS_TEXT'); ?></p></div>
					<div class="card-body border-bottom"><div class="d-flex flex-wrap gap-2">
						<button type="submit" name="task" value="maintenance.verify" class="btn btn-primary"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_VERIFY_SELECTED'); ?></button>
						<button type="submit" name="task" value="maintenance.reanalyse" class="btn btn-outline-primary"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_REANALYSE_SELECTED'); ?></button>
						<button type="submit" name="task" value="maintenance.recalculateChecksums" class="btn btn-outline-secondary" onclick="return confirm(<?php echo htmlspecialchars(json_encode(Text::_('COM_AUDIOARCHIVE_MAINTENANCE_CHECKSUM_CONFIRM')), ENT_QUOTES, 'UTF-8'); ?>);">
					...
					<div class="table-responsive"><table class="table align-middle mb-0"><thead><tr>
						<th scope="col" class="w-1 text-center"><input class="form-check-input" type="checkbox" name="checkall-toggle" value="" title="<?php echo Text::_('JGLOBAL_CHECK_ALL'); ?>" onclick="Joomla.checkAll(this);"></th>
						<th scope="col"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_COLUMN_CLIP'); ?></th>
						<th scope="col"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_COLUMN_FILENAME'); ?></th>
						<th scope="col"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_COLUMN_ISSUES'); ?></th>
					</tr></thead><tbody>
						<?php foreach ($actionableClips as $index => $clip) : ?><tr>
							<td class="text-center"><input class="form-check-input" type="checkbox" id="cb<?php echo (int) $index; ?>" name="cid[]" value="<?php echo (int) $clip['id']; ?>" onclick="Joomla.isChecked(this.checked);"></td>
							<th scope="row"><a href="<?php echo Route::_('index.php?option=com_audioarchive&task=clip.edit&id=' . (int) $clip['id']); ?>"><?php echo htmlspecialchars((string) $clip['title'], ENT_QUOTES, 'UTF-8'); ?></a><div class="small text-body-secondary">#<?php echo (int) $clip['id']; ?></div></th>
							<td><?php echo htmlspecialchars((string) $clip['original_filename'], ENT_QUOTES, 'UTF-8'); ?></td>
							<td><?php foreach ((array) $clip['issues'] as $label) : ?><span class="badge text-bg-secondary me-1 mb-1"><?php echo Text::_((string) $label); ?></span><?php endforeach; ?></td>
						</tr><?php endforeach; ?>
					</tbody></table></div>
				</div>
			<?php endif; ?>
			...
			<div class="card">
				<div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
					<div><h2 class="h4 mb-1"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_REPORT_TITLE'); ?></h2><p class="mb-0 text-body-secondary"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_REPORT_TEXT'); ?></p></div>
					<button type="submit" name="task" value="maintenance.export" class="btn btn-outline-secondary"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_EXPORT'); ?></button>
				</div>
				<?php if ($issues === []) : ?>
					<div class="card-body"><div class="alert alert-success mb-0"><h3 class="h5 alert-heading"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_CLEAN_TITLE'); ?></h3><p class="mb-0"><?php echo Text::_('COM_AUDIOARCHIVE_MAINTENANCE_CLEAN_TEXT'); ?></p></div></div>
				<?php else : ?>
					<div class="table-responsive"><table class="table table-striped align-middle mb-0"><thead><tr>
						...
						<?php foreach ($issues as $issue) : ?>
							...
							<tr>
								<td><span class="badge <?php echo $severityClasses[$severity] ?? 'bg-secondary'; ?>"><?php echo Text::_($severityKeys[$severity] ?? 'COM_AUDIOARCHIVE_MAINTENANCE_SEVERITY_INFO'); ?></span></td>
								<th scope="row"><?php echo Text::_((string) $issue['label']); ?></th>
								<td>
									<?php if ((int) $issue['clip_id'] > 0) : ?>
										<a href="<?php echo Route::_('index.php?option=com_audioarchive&task=clip.edit&id=' . (int) $issue['clip_id']); ?>"><?php echo (string) $issue['clip_title'] !== '' ? htmlspecialchars((string) $issue['clip_title'], ENT_QUOTES, 'UTF-8') : '#' . (int) $issue['clip_id']; ?></a>
										<?php if ((string) $issue['original_filename'] !== '') : ?><div class="small text-body-secondary"><?php echo htmlspecialchars((string) $issue['original_filename'], ENT_QUOTES, 'UTF-8'); ?></div><?php endif; ?>
									<?php else : ?><span class="text-body-secondary"><?php echo Text::_('JNONE'); ?></span><?php endif; ?>
								</td>
								<td><div><?php echo htmlspecialchars((string) $issue['detail'], ENT_QUOTES, 'UTF-8'); ?></div><?php if ((string) $issue['storage_key'] !== '') : ?><code class="d-block mt-1 text-break"><?php echo htmlspecialchars((string) $issue['storage_key'], ENT_QUOTES, 'UTF-8'); ?></code><?php endif; ?></td>
							</tr>
						<?php endforeach; ?>
					</tbody></table></div>
				<?php endif; ?>
