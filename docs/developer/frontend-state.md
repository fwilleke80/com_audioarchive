...
Sound Board performance recordings are stored under `com_audioarchive.soundboard.recordings.v1`. Each recording contains a versioned format identifier, name/creation time, duration, an embedded Sound Board snapshot, initial mode/polyphony/octave/selected-pad state, and a bounded timestamped event array. Events cover pad triggers, chromatic notes with velocity and note duration, mode/polyphony/octave changes, and sampler source-pad selections. Recording playback temporarily replaces only the in-memory board and restores the visitor's previous board/state afterwards; it does not overwrite the personal Sound Board.

Recording JSON import is validated and bounded before storage. During playback, embedded board entries are rechecked through the public Sound Board route-resolution endpoint; UUID mismatches or inaccessible clips are omitted rather than allowing a stale numeric ID to select another clip.
