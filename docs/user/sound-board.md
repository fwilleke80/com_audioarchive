...
During replay, Audio Archive resolves the embedded Sound Board snapshot, temporarily uses it as the in-memory board, and restores the preserved live board/state after playback. Backing events use their own recorded pad/instrument directly and do not take over the live Sound Board selection, octave, mode, or Polyphonic switch. You can therefore select another pad and jam on the chromatic keyboard while a recording is playing. Exported JSON contains the board entries, initial Sound Board state, timing information, and recorded events. Clip IDs are accompanied by their stable UUIDs; when replaying, Audio Archive checks the current public clip metadata so an inaccessible or mismatched clip is not silently substituted.
